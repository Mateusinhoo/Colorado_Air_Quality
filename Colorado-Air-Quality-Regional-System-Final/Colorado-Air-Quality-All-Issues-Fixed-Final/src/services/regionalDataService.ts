...
class RegionalDataService {
  ...
  async getMostPollutedRegions(): Promise<Array<{name: string, regionId: string, aqi: number, population: number}>> {
  ...
  async getCleanestRegions(): Promise<Array<{name: string, regionId: string, aqi: number, population: number}>> {
    const allData = await this.getAllRegionalData();
