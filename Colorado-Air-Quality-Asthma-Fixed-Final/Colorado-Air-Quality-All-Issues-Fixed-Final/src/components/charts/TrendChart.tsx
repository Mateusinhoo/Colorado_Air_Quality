...
const TrendChart: React.FC<TrendChartProps> = ({ selectedRegion, selectedPollutant }) => {
  ...
  const loadTrendData = () => {
    try {
      ...
      if (historicalData.length === 0) {
        ...
        console.log('No historical data found, starting data collection...');
        dataCollectionService.collectDailyRegionalData().then(() => {
          // After collection, try to load data again
          const newData = dataCollectionService.getRegionalHistoricalData(selectedRegion);
          if (newData.length > 0) {
  ...
  useEffect(() => {
    ...
    // Set up interval to refresh data every hour to catch new daily collections
      ...
      <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        ...
        <p className="text-blue-700 dark:text-blue-300 text-sm mt-1">
          {chartData.length === 0 
            ...
            : `${chartData.length} day${chartData.length === 1 ? '' : 's'} of data collected. Chart builds daily at 12:30 AM.`
          }
