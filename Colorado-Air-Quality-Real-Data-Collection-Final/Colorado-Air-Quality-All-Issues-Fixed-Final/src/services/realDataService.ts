...
class RealDataService {
  ...
  async fetchAirQualityData(zip: string): Promise<AirQualityData> {
    try {
      // Use the correct AirNow API endpoint format with smaller distance for more accurate local data
  ...
  // NOTE: This data represents ESTIMATES based on seasonal patterns and regional factors
  // NOT real historical data - for educational/demonstration purposes only
  async getTrendData(zip: string, pollutant: string = 'PM2.5'): Promise<TrendDataPoint[]> {
    ...
    // IMPORTANT: This generates ESTIMATED data based on typical seasonal patterns
    // This is NOT real historical data from EPA or AirNow
    // For real historical data, use EPA's AQS API: https://aqs.epa.gov/aqsweb/documents/data_api.html
    
    const months = ['Jan 2024', 'Feb 2024', 'Mar 2024', 'Apr 2024', 'May 2024', 'Jun 2024', 
                   'Jul 2024', 'Aug 2024', 'Sep 2024', 'Oct 2024', 'Nov 2024', 'Dec 2024'];
    const cityName = this.getCityName(zip);
    
    // Use fixed seed based on zip code for consistent "estimates"
    const seed = parseInt(zip) % 1000;
    ...
    const trendData: TrendDataPoint[] = months.map((month, index) => {
      // Base values that vary by season and location (ESTIMATES ONLY)
      let basePM25 = 35;
      let baseEmergency = 45;
      
      // Seasonal variations (winter typically higher pollution)
      ...
      // City-specific adjustments (ESTIMATES based on typical urban vs rural patterns)
      ...
      // Add consistent variation using seeded random for reproducible "estimates"
      const pm25 = Math.max(15, basePM25 + (seededRandom(index) - 0.5) * 20);
      const emergencyVisits = Math.max(20, baseEmergency + (seededRandom(index + 12) - 0.5) * 30);
      
      return {
        month,
        pm25: Math.round(pm25),
        emergencyVisits: Math.round(emergencyVisits),
        hospitalizations: Math.round(emergencyVisits * 0.4), // Estimated ratio
        asthmaRate: 8.3 // Colorado state average estimate
      };
    });
